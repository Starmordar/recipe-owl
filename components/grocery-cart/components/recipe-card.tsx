...
function RecipeCard({ cartId, cartItem }: RecipeCardProps) {
  return (
    <div className='flex flex-col gap-y-2 pb-4 pt-4 first:pt-0 last:pb-0'>
      <div className='flex flex-row space-y-0 gap-4'>
        <RecipeCardHeader cartId={cartId} recipe={cartItem.recipe} quantity={cartItem.quantity} />
      </div>

      <RecipeCardIngredients<Ingredient>
